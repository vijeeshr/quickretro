...
import {
  areBoardColumnsVisuallySame,
  exceedsEventRequestMaxSize,
  formatDate,
  logMessage,
} from '../utils'
...
import { AvailableLocales, useLanguage } from '../composables/useLanguage'
...
const prevSpotlight = () => {
  ...
  const prevIndex =
    currentIndex === -1 ? users.length - 1 : (currentIndex - 1 + users.length) % users.length
...
const onAdded = (card: DraftMessage) => {
  ...
  dispatchEvent<SaveMessageEvent>('msg', {
    id: card.id,
...
const otherActiveUsersCount = computed(() => onlineUsersCardsStats.value.length)
...
const onCategoryChanged = (pyl: CategoryChangeMessage) => {
  ...
  dispatchEvent<CategoryChangeEvent>('catchng', {
    ...
    newcat: pyl.newCategoryId,
    oldcat: pyl.oldCategoryId,
  })
}
...
const saveCategoryChanges = () => {
  const enabledCols: BoardColumn[] = mergedCategories.value
...
const onColumnsChangeResponse = (response: ColumnsChangeResponse) => {
  columns.value = response.columns.slice().sort((a, b) => a.pos - b.pos)
...
const hasCardsInDisabledCategories = computed(() => {
  ...
  const disabledCatIds = mergedCategories.value.filter(cat => !cat.enabled).map(cat => cat.id)
...
const mergeCategories = () => {
  ...
  const merged = defaultCats.map(d => {
...
<template>
  <div class="flex h-full min-h-screen bg-gray-800 dark:bg-gray-950 text-white">
    ...
    >
      ...
      <Avatar
        v-if="spotlightFor?.byxid !== '' && spotlightFor?.nickname !== ''"
        class="min-w-32"
        :name="spotlightFor?.nickname"
        view-type="Badge"
      />
      <div
        v-else
      ...
      >
        <span class="font-medium text-xs cursor-default text-gray-600 select-none">{{
          t('common.anonymous')
        }}</span>
      </div>
    ...
    <Dialog :open="isTimerDialogOpen" class="relative z-60" @close="setIsTimerDialogOpen">
      ...
      <div class="fixed inset-0 flex w-screen items-center justify-center p-4">
        <DialogPanel
          class="rounded-2xl bg-white dark:bg-gray-800 p-6 text-left align-middle shadow-xl"
        >
          <DialogTitle
            as="h3"
          ...
          <TimerPanel
            :is-countdown-in-progress="isTimerCountdownInProgress"
            @start="onTimerStart"
            @stop="onTimerStop"
          ></TimerPanel>
        </DialogPanel>
    ...
    <Dialog :open="isColumnEditDialogOpen" class="relative z-60" @close="setIsColumnEditDialogOpen">
      ...
      <div class="fixed inset-0 flex w-screen items-center justify-center p-4">
        ...
        >
          ...
          <p
            v-show="hasCardsInDisabledCategories"
            class="text-sm text-red-600 dark:text-red-300 mt-2 select-none"
          >
            {{ t('dashboard.columns.cannotDisable') }}
          </p>
          <button
            type="button"
    ...
    <div class="flex-1 min-w-0 min-h-screen overflow-clip">
      ...
      <div
        v-if="isLocked"
        class="sticky top-0 z-51 flex items-center justify-center gap-4 h-10 w-full bg-amber-50/80 dark:bg-amber-900/40 backdrop-blur-md border-b border-amber-200/20 dark:border-amber-800/30 transition-all"
      >
        <div class="flex items-center text-amber-800 dark:text-amber-200 text-sm font-medium">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
            fill="currentColor"
        ...
        <button
          v-if="isOwner"
          class="text-[10px] uppercase tracking-widest font-bold px-2 py-1 bg-amber-600 hover:bg-amber-700 text-white rounded transition-colors"
          @click="unlock"
        >
          {{ t('dashboard.lock.unlockButton') }}
        </button>
      </div>
      <div
      ...
      >
        ...
        >
          ...
          />
          <NewAnonymousCard
            v-if="newAnonymousCardCategory == column.id"
            :category="column.id"
            nickname=""
            @added="onAdded"
            @invalid-content="onInvalidContent"
